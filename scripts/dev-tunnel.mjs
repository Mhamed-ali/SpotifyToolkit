...
async function startTunnelAndServer() {
  try {
    const targetSubdomain = 'spotify-toolkit-dev';
    const expectedUrl = `https://${targetSubdomain}.loca.lt`;
    const baseDelayMs = 5000; // Configurable base wait time (x)
    let tunnel = null;
    let attempts = 0;
  ...
  console.log(`===================================================\n`);
    ...
    // 2. Update .env.local dynamically
    ...
    const nextProcess = spawn('npm', ['run', 'dev'], { 
      cwd: rootDir,
      stdio: 'inherit',
      shell: true // Important for Windows to resolve 'npm.cmd'
    });
